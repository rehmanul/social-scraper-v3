...
// Also check for Bearer Tokens (user provided specific vars)
...
if (credentials.appKey && credentials.appSecret) {
    console.log('[Twitter API] Initializing with User Context (OAuth 1.0a)');
    client = new TwitterApi(credentials);
    roClient = client.readOnly;
} else if (bearerToken) {
    console.log('[Twitter API] Initializing with App Context (Bearer Token)');
...
export async function fetchUserTweets(username, count = 30) {
    ...
    if (!roClient) {
        console.error('[Twitter API] Missing credentials. Set TWITTER_BEARER_TOKEN or API keys.');
