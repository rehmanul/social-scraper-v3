...
export async function fetchVideo(videoId) {
    ...
    try {
        ...
        // Extract exact upload date for epoch from microformat or regex
        let epochTime = 0;
        try {
            // Try 1: Regex in the HTML (safest and fastest)
            const uploadDateMatch = html.match(/"uploadDate":"([^"]+)"/);
            if (uploadDateMatch && uploadDateMatch[1]) {
                epochTime = Math.floor(new Date(uploadDateMatch[1]).getTime() / 1000);
            } else {
                // Try 2: Look in ytInitialPlayerResponse
                // This variable often contains the microformat data for the player
                ...
                if (playerRespMatch) {
                    ...
                    if (microformat && microformat.uploadDate) {
                        epochTime = Math.floor(new Date(microformat.uploadDate).getTime() / 1000);
                    }
                }

                // Try 3: Look in ytInitialData (already parsed)
                if (!epochTime && data) {
            ...
            console.log('[YouTube Scraper] Failed to parse date:', e.message);
        }
        ...
        return {
            ...
            published_at: dateText, // Relative text like "2 days ago"
            epoch_time_posted: epochTime,
            channel: {
                name: channelName,
                id: channelId
            },
