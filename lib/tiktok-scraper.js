// lib/tiktok-scraper.js - Lightweight TikTok scraper without native dependencies
// Uses TikTok's web API directly + SIGI_STATE parsing
// Fallback when parse.bot API fails

import axios from 'axios';

const USER_AGENT = 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/120.0.0.0 Safari/537.36';

/**
 * Fetch user posts by scraping profile page
 */
export async function fetchUserPosts(username, count = 30) {
    const cleanUsername = username.replace('@', '').trim();
    console.log(`[TikTok Scraper] Fetching posts for @${cleanUsername}`);

    try {
        const profileUrl = `https://www.tiktok.com/@${cleanUsername}`;
        const pageResponse = await axios.get(profileUrl, {
            headers: {
                'User-Agent': USER_AGENT,
                'Accept': 'text/html,application/xhtml+xml',
                'Accept-Language': 'en-US,en;q=0.9',
            },
            timeout: 30000
        });

        const html = pageResponse.data;
        let posts = [];
        let user = null;
        let secUid = null;

        // Extract SIGI_STATE JSON data from page
        const sigiMatch = html.match(/<script id="SIGI_STATE"[^>]*>(.+?)<\/script>/s);

        if (sigiMatch) {
            try {
                const sigiState = JSON.parse(sigiMatch[1]);
                const itemModule = sigiState.ItemModule || {};
                const userModule = sigiState.UserModule || {};

                // Get user info
                const users = Object.values(userModule.users || {});
                if (users.length > 0) {
                    user = users[0];
                    secUid = user.secUid;
                }

                // Get video posts
                posts = Object.values(itemModule);

                // Sort by createTime (newest first)
                posts.sort((a, b) => b.createTime - a.createTime);

                // Limit count
                posts = posts.slice(0, count);

            } catch (e) {
                console.log('[TikTok Scraper] Failed to parse SIGI_STATE:', e.message);
                throw new Error('Failed to parse TikTok page content');
            }
        } else {
            console.log('[TikTok Scraper] SIGI_STATE not found');
            // Try __UNIVERSAL_DATA_FOR_REHYDRATION__
            const univMatch = html.match(/<script id="__UNIVERSAL_DATA_FOR_REHYDRATION__"[^>]*>(.+?)<\/script>/s);
            if (univMatch) {
                try {
                    const univData = JSON.parse(univMatch[1]);
                    const userDetail = univData?.__DEFAULT_SCOPE__?.['webapp.user-detail'];

                    if (userDetail) {
                        if (userDetail.userInfo && userDetail.userInfo.user) {
                            user = userDetail.userInfo.user;
                            secUid = user.secUid;
                        }

                        if (userDetail.itemStruct) {
                             posts = userDetail.itemStruct;
                        } else if (userDetail.itemList) {
                             posts = userDetail.itemList;
                        }
                    }
                } catch (e) {
                    console.log('[TikTok Scraper] Failed to parse UNIVERSAL_DATA:', e.message);
                }
            }
        }

        // If no posts found but we have secUid, try the API
        if (posts.length === 0 && secUid) {
            console.log('[TikTok Scraper] No posts in HTML, trying API with secUid:', secUid);
            try {
                // The API seems to limit count to 35. We cap it at 30 to be safe.
                const safeCount = Math.min(count, 30);
                const apiResponse = await axios.get('https://www.tiktok.com/api/post/item_list/', {
                    params: {
                        aid: 1988,
                        count: safeCount,
                        secUid: secUid,
                        cursor: 0
                    },
                    headers: {
                        'User-Agent': USER_AGENT,
                        'Referer': profileUrl,
                        'Cookie': 'tt_webid_v2=1234567890'
                    },
                    timeout: 10000
                });

                if (apiResponse.data && apiResponse.data.itemList) {
                    posts = apiResponse.data.itemList;
                    console.log(`[TikTok Scraper] API returned ${posts.length} videos`);
                } else {
                    console.log('[TikTok Scraper] API returned no videos');
                }
            } catch (e) {
                console.log('[TikTok Scraper] API fallback failed:', e.message);
                if (e.response) {
                    console.log('Error details:', e.response.status, e.response.data);
                }
            }
        }

        if (posts.length === 0) {
            throw new Error('No videos found (Anti-scraping protection likely active)');
        }

        console.log(`[TikTok Scraper] Found ${posts.length} videos`);

        // Map posts to a common structure
        // Note: The structure from API and from HTML might differ slightly, we need to handle that.
        // The transform function expects: id, desc, createTime, stats, author, video, music

        const mappedPosts = posts.map(post => {
            return {
                id: post.id || post.video?.id, // ID might be at root or under video
                desc: post.desc,
                createTime: post.createTime,
                stats: post.stats,
                author: post.author || user, // Use post author or fallback to profile user
                video: post.video,
                music: post.music
            };
        });

        return {
            videos: mappedPosts,
            user: user,
            source: 'page_scraping'
        };

    } catch (error) {
        console.error(`[TikTok Scraper] Error:`, error.message);
        throw error;
    }
}

/**
 * Fetch single video details by URL
 */
export async function fetchVideo(videoUrl) {
    console.log(`[TikTok Scraper] Fetching video: ${videoUrl}`);

    try {
        const response = await axios.get(videoUrl, {
            headers: {
                'User-Agent': USER_AGENT,
                'Accept': 'text/html,application/xhtml+xml',
                'Accept-Language': 'en-US,en;q=0.9',
            },
            timeout: 30000
        });

        const html = response.data;
        let videoData = null;

        // Try SIGI_STATE
        const sigiMatch = html.match(/<script id="SIGI_STATE"[^>]*>(.+?)<\/script>/s);
        if (sigiMatch) {
            try {
                const sigiState = JSON.parse(sigiMatch[1]);
                const itemModule = sigiState.ItemModule || {};

                // Usually there is only one item in ItemModule for a video page
                const videos = Object.values(itemModule);
                if (videos.length > 0) {
                    videoData = videos[0];
                }
            } catch (e) {
                console.log('[TikTok Scraper] Failed to parse SIGI_STATE for video:', e.message);
            }
        }

        // Try __UNIVERSAL_DATA_FOR_REHYDRATION__
        if (!videoData) {
            const univMatch = html.match(/<script id="__UNIVERSAL_DATA_FOR_REHYDRATION__"[^>]*>(.+?)<\/script>/s);
            if (univMatch) {
                try {
                    const univData = JSON.parse(univMatch[1]);
                    // Path to video detail might vary
                    videoData = univData?.__DEFAULT_SCOPE__?.['webapp.video-detail']?.itemInfo?.itemStruct;
                } catch (e) {
                    console.log('[TikTok Scraper] Failed to parse UNIVERSAL_DATA for video:', e.message);
                }
            }
        }

        if (!videoData) {
             throw new Error('Video data not found in page');
        }

        return {
            video_id: videoData.id,
            url: videoUrl,
            description: videoData.desc || '',
            epoch_time_posted: videoData.createTime || 0,
            views: videoData.stats?.playCount || 0,
            likes: videoData.stats?.diggCount || 0,
            comments: videoData.stats?.commentCount || 0,
            shares: videoData.stats?.shareCount || 0,
            cover_image: videoData.video?.cover || '',
            video_url: videoData.video?.playAddr || '',
            author: {
                id: videoData.author?.id || '',
                username: videoData.author?.uniqueId || '',
                nickname: videoData.author?.nickname || '',
                avatar: videoData.author?.avatarThumb || '',
            },
            music: {
                id: videoData.music?.id || '',
                name: videoData.music?.title || '',
                author: videoData.music?.authorName || '',
            }
        };

    } catch (error) {
        console.error(`[TikTok Scraper] Error fetching video ${videoUrl}:`, error.message);
        return null;
    }
}

/**
 * Transform scraped data to standard format
 */
export function transformScrapedVideos(rawData, username, page = 1, perPage = 10) {
    const requestTime = Math.floor(Date.now() / 1000);
    const videos = rawData?.videos || [];

    const transformed = videos.map(v => ({
        video_id: v.id,
        url: `https://www.tiktok.com/@${username}/video/${v.id}`,
        description: v.desc || '',
        epoch_time_posted: v.createTime || 0,
        views: v.stats?.playCount || 0,
        likes: v.stats?.diggCount || 0,
        comments: v.stats?.commentCount || 0,
        shares: v.stats?.shareCount || 0,
        cover_image: v.video?.cover || '',
        video_url: v.video?.playAddr || '',
        author: {
            id: v.author?.id || '',
            username: v.author?.uniqueId || username,
            nickname: v.author?.nickname || '',
            avatar: v.author?.avatarThumb || '',
        },
        music: {
            id: v.music?.id || '',
            name: v.music?.title || '',
            author: v.music?.authorName || '',
        }
    }));

    const totalPosts = transformed.length;
    const totalPages = Math.ceil(totalPosts / perPage);
    const startIndex = (page - 1) * perPage;
    const paginated = transformed.slice(startIndex, startIndex + perPage);

    return {
        meta: {
            username: rawData?.user?.uniqueId || username,
            page,
            total_pages: totalPages,
            posts_per_page: perPage,
            total_posts: totalPosts,
            request_time: requestTime,
            source: 'page_scraping'
        },
        data: paginated,
        status: 'success'
    };
}
