...
/**
 * Fetch specific videos by ID (comma separated)
 */
export async function fetchVideosByIds(videoIds) {
    ...
    console.log(`[YouTube Official API] Fetching details for ${ids.length} videos`);

    // If no API key, fall back to scraping each video
    if (!YOUTUBE_API_KEY) {
        console.log('[YouTube Official API] No API key, scraping videos individually');
        const { fetchVideo: scrapeFetchVideo } = await import('./youtube-scraper.js');

        // Fetch in parallel
        const promises = ids.map(id => scrapeFetchVideo(id));
    ...
    try {
        ...
        // Note: API allows max 50 IDs per request. If more, we should chunk it.
        // Assuming reasonably small batches for now.
        ...
        return items.map(item => ({
            ...
            epoch_time_posted: item.snippet.publishedAt ? Math.floor(new Date(item.snippet.publishedAt).getTime() / 1000) : 0,
        ...
        console.error(`[YouTube Official API] Error fetching videos:`, error.message);

        // Fallback to scraper on error? Or just throw?
        // Fallback is safer
        console.log('[YouTube Official API] Falling back to scraper due to error');
        const { fetchVideo: scrapeFetchVideo } = await import('./youtube-scraper.js');
        const promises = ids.map(id => scrapeFetchVideo(id));
        const results = await Promise.all(promises);
        return results.filter(r => r !== null);
...
export default { fetchChannelVideos, fetchVideosByIds, transformVideos };
